...
const Login = () => {
  ...
  const handleSendOtp = async () => {
    try {
      await axios.post("https://anadhsevabackend-3.onrender.com//api/otp", {
        email,
      });
      setOtpSent(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "https://anadhsevabackend-3.onrender.com//api/otpVerify",
        { email, otp }
      );
      toast.success("Login successful.");
      const response = await axios.post(
        "https://anadhsevabackend-3.onrender.com//api/auth/login",
        { email }
      );
